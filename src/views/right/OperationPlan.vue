        ...
        >
          <template #bodyCell="{ column, text, record, index }">
            <div v-if="column.dataIndex === 'source'">
              {{ global.$isNotEmpty(text) ? global.$store.state.dictionary.source.find((item: any) => item.value ===
                text).title : '' }}
            </div>
            <div
...
import { screenOpPlanRequest } from '@/api/screen';
...
const getData = () => {
  screenOpPlanRequest({
    ...
    .then((response: any) => {
      console.log('=====screenOpPlanRequest=====');
...
onMounted(() => {
  // new PerfectScrollbar('.ant-table-tbody')
});
</script>
